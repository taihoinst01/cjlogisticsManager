...
function searchDialog(dlgID, missingEntitiesData) {
    ...
    $.ajax({
        ...
        success: function (result) {          //성공했을 때 함수 인자 값으로 결과 값 나옴
            ...
            if (result['list'].length == 0) {
                ...
                var row = result['list'];
                alert("missing=="+result['list'][0].MissingEntities);
                ...
                for (var i = 0; i < row.length; i++) {
                    ...
                    for (var jj = 0; jj < tmp.MissingEntities.length; jj++) {
                        missingEntitySelectHtml += '<option value="' + tmp.MissingEntities[jj].MISSINGENTITIES + '">' + tmp.MissingEntities[jj].MISSINGENTITIES + '</option>';
                    }

                }
            }
            $('.dialogView').html(inputUttrHtml);
            alert("missingEntitySelectHtml==="+missingEntitySelectHtml);
            $("#missingEntities").html(missingEntitySelectHtml);
    ...
    */
            ...
            var $inputMissingEntities = $('input[name=missingEntities]');
            var $selectMissingEntities = $('select[name=missingEntities]');

            $selectMissingEntities.show();
            $selectMissingEntities.removeAttr('disabled');

            $inputMissingEntities.hide();
            $inputMissingEntities.attr('disabled', 'disabled');
            ...
            $("#missingEntities").val(missingEntitiesData).prop("selected", true);
